...
class Aws:
    ...
    async def get_credentials(
        ...
        # transitive tags are used to track identity information across role assumptions
        transitive_user_tag_key = config.get('aws.transitive_user_tag_key', None)
        if transitive_user_tag_key:
            session_tags = [
                {
                    'Key': transitive_user_tag_key,
                    'Value': user.lower()
                },
            ]
            transitive_tag_keys = [transitive_user_tag_key]
        else:
            session_tags = []
            transitive_tag_keys = []
